...
from .decorators import lecturer_required, student_required, admin_required,parent_required
...
def student_add_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            parent_id=Parent.objects.filter(user_id=request.user.id).first().id
...
@login_required
@admin_required
def student_admin_add_view(request):
    if request.method == "POST":
        form = StudentAdminAddForm(request.POST)
        first_name = request.POST.get("first_name")
        last_name = request.POST.get("last_name")
        if form.is_valid():
        ...
        form = StudentAdminAddForm()

    return render(
        request,
        "accounts/admin_add_student.html",
        {"title": "Add Student | DjangoSMS", "form": form},
    )

@login_required
@admin_required
def group_admin_add_view(request):
    groups = Student_Group.objects.all().order_by('created_date')
    if request.method=='POST':
        form=StuGroupAddForm(request.POST)
        if form.is_valid():
            selected_students = form.cleaned_data['students']
            
            group = form.save()
            for student in selected_students:
                stu = Student.objects.filter(id=student.pk).first()
                stu.student_group_id=group.id
                stu.save()
        ...
        form = StuGroupAddForm()

    return render(
        request,
        "accounts/add_group.html",
        {"title": "Add Group | DjangoSMS", "form": form,"groups":groups},
    )

@login_required
@admin_required
def group_edit(request, pk):
    group = get_object_or_404(Student_Group, pk=pk)
    if request.method=='POST':
        form=StuGroupAddForm(request.POST)
        if form.is_valid():
            selected_students = form.cleaned_data['students']
            
            group.student_group.set(selected_students)
            group.save()
        ...
        initial_selected_stds=group.student_group.all()
        form = StuGroupAddForm(instance=group,initial={"students":initial_selected_stds})

    return render(
        request,
        "accounts/edit_group.html",
        {"title": "Add Group | DjangoSMS", "form": form,},
    )
...
def edit_student(request, pk):
    ...
    if request.method == "POST":
        form = StudentProfileUpdateForm(request.POST, request.FILES, instance=instance)
        full_name = instance.get_full_name
        if form.is_valid():
            
            student.session_hours = form.cleaned_data.get("session_hours")
            student.student_group = form.cleaned_data.get('student_group')
        ...
        form = StudentProfileUpdateForm(instance=instance,initial={"session_hours": student.session_hours,"department":student.department,"student_group":student.student_group,"parent":student.parent})
    return render(
        request,
...
@login_required
@parent_required
def edit_children(request, pk):
    instance = get_object_or_404(User, is_student=True, pk=pk)
    if request.method == "POST":
        form = parentUpdateChildrenForm(request.POST, request.FILES, instance=instance)
        full_name = instance.get_full_name
        if form.is_valid():
            form.save()
        ...
        form = StudentProfileUpdateForm(instance=instance,)
    return render(
        request,
        "accounts/edit_children.html",
        {
            "title": "Edit-profile | DjangoSMS",
            "form": form,
        },
...
class StudentListView(ListView):
    ...
    def get_queryset(self):
        ...
        parent_id=Parent.objects.filter(user_id=user_id).first()
        if self.request.user.is_parent:
            queryset = Student.objects.filter(parent_id=parent_id).order_by('id')
...
def delete_student(request, pk):
    ...
    user = get_object_or_404(User, id=student.student_id)
    # full_name = student.user.get_full_name
    student.delete()
    user.delete()
...
def parent_delete(request, pk):
    ...
    # full_name = student.user.get_full_name
    parent.delete()
    messages.success(request, "parent has been deleted.")
