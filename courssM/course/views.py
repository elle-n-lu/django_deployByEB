...
from accounts.models import User, Student,Student_Group
...
def program_detail(request, pk):
    ...
    if name_filter:
        courses = Course.objects.filter(title__icontains=name_filter,year__icontains=year_filter,
...
def course_add(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, (course_name  + ' has been created.'))
...
# ########################################################
@login_required
def CourseAllocationFormView(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            course_length = form.cleaned_data['course_length']

            if student:
               t= CourseAllocation(content_object=student,lecturer=lecturer,courses=selected_courses,course_length=course_length)
               t.save()
            if group:
               t= CourseAllocation(content_object=group,lecturer=lecturer,courses=selected_courses,course_length=course_length)
               t.save()
...
def course_allocation_view(request):
    ...
    allocated_courses = CourseAllocation.objects.all()
    if lecturer_filter:
        allocated_courses=allocated_courses.filter(lecturer__first_name__icontains=lecturer_filter)
    if student_filter:
        if studentOrGroup._meta.model.__name__=='User':
            allocated_courses=allocated_courses.filter(content_objects_user=studentOrGroup)
        if studentOrGroup._meta.model.__name__=='Student_Group':
            allocated_courses=allocated_courses.filter(content_object_group=studentOrGroup)

    if course_filter:
        allocated_courses=allocated_courses.filter(courses__title__icontains=course_filter)
...
def edit_allocated_course(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            group = form.cleaned_data['group']
            course_length = form.cleaned_data['course_length']
            print('yt',type(lecturer),type(User.objects.filter(id=lecturer.id)))
            if student:

                allocated.content_type.model_class=student
                allocated.object_id=student.id
                allocated.lecturer=lecturer
                allocated.courses=selected_courses
                allocated.course_length=course_length
                allocated.save()
            if group:
                allocated.content_type.model_class=group
                allocated.object_id=group.id
                allocated.lecturer=User.objects.filter(id=lecturer.id).first()
                allocated.courses=selected_courses
                allocated.course_length=course_length
                allocated.save()
        ...
        initial_data = {
            "lecturer":allocated.lecturer,
            "courses" :allocated.courses,
            "course_length":allocated.course_length
        }
        if allocated.content_type.model_class().__name__ == 'User':
           initial_data['student'] = allocated.content_object
        if allocated.content_type.model_class().__name__ == 'Student_Group':
           initial_data['group'] = allocated.content_object
        form = CourseAllocationForm(initial=initial_data)
