...
class Course(models.Model):
    ...
    semester = models.CharField(choices=SEMESTER, max_length=200, blank=True,null=True,)
...
class CourseAllocationManager(models.Manager):
    def search(self, query=None):
        qs = self.get_queryset()
        if query is not None:
            or_lookup = (Q(lecturer__first_name__icontains=query) | 
...
class CourseAllocation(models.Model):
    ...
    course_length = models.FloatField(default=0, null=True,blank=True)
    
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE)
    object_id = models.PositiveIntegerField()
    content_object = GenericForeignKey('content_type', 'object_id')

    courses = models.ForeignKey(Course,on_delete=models.CASCADE, related_name='allocated_course')
    session = models.ForeignKey("app.Session", on_delete=models.CASCADE, blank=True, null=True)
    created_time=models.DateTimeField(auto_now_add=True)
    objects=CourseAllocationManager()

    def __str__(self):
        return f"{self.lecturer.get_full_name} - {self.content_object} - {self.courses.title}"
